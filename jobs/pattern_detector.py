...
def detect_patterns_for_symbol(symbol, name, rows):
    ...
    win20 = rows[-20:]
    prev20 = rows[-40:-20]
    support = min(nf(r.get("low")) for r in win20 if nf(r.get("low")) is not None)
    resistance = max(nf(r.get("high")) for r in win20 if nf(r.get("high")) is not None)
    close = closes[-1]
    prev_close = closes[-2]
    ...
    recent_pullback = pct(close, max(closes[-12:]))
    ...
    low1 = min(lows40[:20])
    low2 = min(lows40[20:])
    if abs(low1 - low2) / close * 100 < 4 and close > ma(closes, 20):
        out.append(_row(symbol, name, latest, "W底", 73, min(low1, low2), resistance, "兩次測底未破且重新站回均線。", "頸線未突破前仍可能回測。"))
    highs40 = highs[-40:]
    high1 = max(highs40[:20])
    high2 = max(highs40[20:])
    if abs(high1 - high2) / close * 100 < 4 and close < ma(closes, 20):
        out.append(_row(symbol, name, latest, "M頭", 72, support, max(high1, high2), "兩次攻高失敗且收回均線下方。", "若重新站回頸線，空方訊號失效。"))

    left = max(highs[-55:-38])
    head = max(highs[-38:-18])
    right = max(highs[-18:])
    if head > left * 1.03 and head > right * 1.03 and close < ma(closes, 20):
        out.append(_row(symbol, name, latest, "頭肩頂", 68, support, head, "中段高點明顯高於左右肩，且短線轉弱。", "型態需跌破頸線才完全確認。"))
    left_low = min(lows[-55:-38])
    head_low = min(lows[-38:-18])
    right_low = min(lows[-18:])
    if head_low < left_low * 0.97 and head_low < right_low * 0.97 and close > ma(closes, 20):
        out.append(_row(symbol, name, latest, "頭肩底", 68, head_low, resistance, "中段低點明顯低於左右肩，且重新站上短均。", "需突破頸線並守住才較可靠。"))

    prev_res = max(nf(r.get("high")) for r in prev20 if nf(r.get("high")) is not None)
    prev_sup = min(nf(r.get("low")) for r in prev20 if nf(r.get("low")) is not None)
    if nf(latest.get("high")) > prev_res and close < prev_res:
        out.append(_row(symbol, name, latest, "假突破", 78, support, prev_res, "盤中突破前高但收盤跌回壓力下。", "追價容易被套，需等待重新站穩。"))
    if close > prev_res and vol > avg_vol20 * 1.5:
